    ...
    <form class="login-form" method="POST" action="{{ route('register') }}">
      ...
      <div class="row">
        <div class="input-field col s12">
          <h5 class="ml-4">{{ __('Cadastrar-se') }}</h5>
          <p class="ml-4">{{ __('Tecnologia do futuro, para o se presente') }}</p>
        </div>
      </div>
      ...
      <div class="row">
        <div class="input-field col s12">
          <button type="submit"
            class="btn waves-effect waves-light border-round gradient-45deg-purple-deep-orange col s12">{{ __('Cadastrar-se') }}</button>
        </div>
      </div>
      <div class="row">
        <div class="input-field col s12">
          <p class="margin medium-small"><a href="{{ route('login')}}">{{ __('Já tem conta? Entre!') }}</a></p>
        </div>
      </div>
    </form>
  </div>
